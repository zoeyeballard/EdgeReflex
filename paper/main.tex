...
\begin{abstract}
Real-time embedded systems increasingly require agentic AI inference loops that carry state between inference steps, yet this fundamental departure from stateless classifiers significantly tightens period constraints in ways that software alone cannot accommodate. This work presents EdgeReflex, a FreeRTOS-based Human Activity Recognition system deployed on a TM4C123GH6PM microcontroller, that demonstrates the complete design methodology for schedulable agentic inference. We employ formal Worst-Case Execution Time (WCET) measurement via ARM Cortex-M4 DWT cycle counters, Rate Monotonic schedulability analysis, and clearly identify the agentic period requirement as the system's breaking point. When the agentic feedback loop is introduced with a 100~ms period constraint, system utilization rises from 0.10 to 3.25, causing deadline misses in both the Logger and Feedback tasks. Response Time Analysis confirms these failures. We then propose and validate a hardware-software co-design solution via FPGA MAC offload that restores schedulability. Results show that projected scheduling constraints are satisfied following Layer 1 inference offload to an Intel Cyclone~V FPGA, with measured inference timing now showing highly deterministic behavior (min 25.39~ms, mean 25.39~ms, max 25.40~ms at 80~MHz across 1012 samples). This work demonstrates a complete, methodology-driven path from formal WCET measurement through schedulability proof to hardware acceleration, offering a reusable template for embedded agentic AI systems beyond HAR.
\end{abstract}

\begin{IEEEkeywords}
real-time scheduling, Rate Monotonic analysis, WCET measurement, agentic inference, embedded AI, FPGA acceleration, hardware-software co-design
\end{IEEEkeywords}

\section{Introduction}

Edge artificial intelligence is rapidly evolving from stateless inference toward agentic loops that carry persistent state between decision steps. While traditional classifiers process each input independently, agentic systems maintain feedback: the output of one inference step becomes part of the input to the next step. This architectural shift introduces a fundamental timing challenge that has received limited formal treatment in the embedded systems literature.

In rate-monotonic (RM) scheduled real-time systems, task periods are the primary schedulability lever. Stateless inference tasks can tolerate generous periods (e.g., 1~s) without degradation, but agentic loops require tighter periods to maintain state freshness and behavioral continuity. When a human activity recognition (HAR) agent tracks motion patterns, stale state vectors beyond 100~ms can degrade classification quality. This creates a hard deadline: reduce the period from 1~s to 100~ms, and suddenly utilization climbs from acceptable to unfeasible.

Rate Monotonic scheduling, formalized by Liu and Layland in 1973~\cite{Liu1973}, provides a rigorous framework for proving real-time feasibility when task periods, deadlines, and execution times are known. However, measuring WCET in modern embedded AI systems has proven challenging. Prior work on embedded AI acceleration either applies scheduling theory to synthetic workloads, or demonstrates FPGA speedup without formal schedulability guarantees. This work bridges that gap.
...
\item Proposes and validates a hardware-software co-design solution via FPGA MAC offload that restores full schedulability.
...
\textbf{Edge AI and Embedded Learning:} Warden and Situnayake~\cite{Warden2019} provide comprehensive treatment of TensorFlow Lite on microcontrollers, focusing on memory footprint and inference latency. Ramegowda and Lin~\cite{Ramegowda2022} examine mixed real-time task handling on FreeRTOS, but without agentic state extensions. Pang et al.~\cite{Pang2022} analyze deep learning on UCI HAR data, achieving high accuracy, but do not address real-time constraints.

\textbf{WCET Analysis and Measurement:} Wilhelm et al.~\cite{Wilhelm2008} survey WCET analysis methods and tools. Abella et al.~\cite{Abella2015} discuss pitfalls in WCET trustworthiness, particularly for complex microarchitectures. Biswas et al.~\cite{Biswas2021} specifically address ARM Cortex performance counters for control flow integrity, validating the use of DWT counters for cycle counting.

\textbf{Gap in Literature:} Few works simultaneously apply formal RM analysis to neural inference tasks with agentic state extensions on microcontrollers, provide rigorous WCET measurement, and demonstrate a hardware-software co-design path to schedulability. This work fills that gap.
...
The state machine operates in five phases:
\begin{enumerate}
...
\item \textbf{SENDING:} Clocking out 64 result bytes via MISO.
\item \textbf{IDLE:} Pulsing IRQ to signal completion; awaiting next cycle.
\end{enumerate}
